...
export function EmptyChat() {
  return (
    <div className="flex-1 h-full flex flex-col items-center justify-center bg-[var(--chat-bg)] p-6 md:p-10 animate-in relative overflow-hidden">
      ...
      {/* Central Shield Logo */}
      <div className="relative mb-10 md:mb-12 shield-animate z-10">
        <div className="w-[120px] h-[120px] md:w-[140px] md:h-[140px] rounded-full bg-[var(--primary)]/5 flex items-center justify-center border border-[var(--primary)]/10 backdrop-blur-sm">
          <div className="w-[90px] h-[90px] md:w-[100px] md:h-[100px] rounded-full bg-[var(--primary)]/10 flex items-center justify-center border border-[var(--primary)]/20 relative">
            <Shield className="w-12 h-12 md:w-14 md:h-14 text-[var(--primary)]" strokeWidth={1.2} />
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[var(--chat-bg)] p-1.5 rounded-lg border border-white/10 shadow-xl">
              <Lock className="w-4 h-4 md:w-5 md:h-5 text-white" />
            </div>
          </div>
        </div>
        {/* Glow effect */}
        <div className="absolute inset-0 rounded-full bg-[var(--primary)]/20 blur-[50px] md:blur-[60px] -z-10" />
      </div>

      <div className="text-center max-w-[500px] mb-10 md:mb-12 z-10 px-6">
        <h2 className="text-[28px] md:text-[32px] font-black text-white mb-4 tracking-tight">
        ...
        <p className="text-[15px] md:text-[16px] text-[var(--text-secondary)] leading-relaxed">
          ...
          Mọi dữ liệu được mã hóa đầu-cuối <span className="text-[var(--primary)] font-bold">(E2EE)</span> và chỉ những người trong cuộc mới có quyền truy cập.
        </p>
      </div>

      {/* Feature Cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-[600px] z-10 px-6">
